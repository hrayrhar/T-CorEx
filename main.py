...
class Corex(object):
    ...
    def fit(self, x):
        ...
        self.last_update = [0 for t in range(self.nt)]  # Used for momentum methods
    ...
    def update_records(self, moments, delta):
        ...
        for t in range(self.nt):
            self.history[t]["TC"] = self.historyp[t].get("TC", []) + [moments[t]["TC"]]
        if self.verbose > 1:
    ...
    def _update_ns(self, x):
        ...
        for t in range(self.nt):
            ...
            sig_grad = self._sig(x[t], grad)
            Bj = np.sum(m[t]["rho"] * grad, axis=1, keepdims=True)
            update = - rj * (grad - 2. * self.ws[t] / (2 - rj) * Bj)  # Gamma Hess^-1 Grad
            ...
            while backtrack:
                if eta < min(self.tol, 1e-10):
                    ...
                    break
                w_update = self.ws[t] + eta * update
                m_update = self._calculate_moments_ns(x[t], w_update, quick=True)
                if not m_update:  # TEST 1: Make sure rho is a valid solution, if not m_update will return False
                    eta *= 0.5
                    if self.verbose > 1:
                        print(('back:{:.7f}'.format(eta)))
                    continue
                wolfe1 = -m_update['TC'] <= -m[t]['TC'] + 0.1 * eta * update_tangent
            ...
            m_updates.append(m_update)

        return w_updates, m_updates
    ...
    def invert(self, x):
        ...
        for t in range(len(x)):
            if self.gaussianize == 'standard':
                ret[t] = self.theta[t][1] * x[t] + self.theta[t][0]
            elif self.gaussianize == 'outliers':
                ret[t] = self.theta[t][1] * g_inv(x[t]) + self.theta[t][0]
            else:
                ret[t] = x[t]
        return ret
